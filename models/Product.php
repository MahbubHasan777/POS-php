    ...
    public function save($data, $shop_id, $id = null)
    {
        if ($id) {
            $this->query(
                ...
                [$data['name'], $data['category_id'], $data['brand_id'], $data['buy_price'], $data['sell_price'], $data['stock_qty'], $data['alert_threshold'], $data['image'], $id, $shop_id],
                "siidddissi"
            );
        } else {
            $this->query(
                "INSERT INTO products (shop_id, name, category_id, brand_id, buy_price, sell_price, stock_qty, alert_threshold, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [$shop_id, $data['name'], $data['category_id'], $data['brand_id'], $data['buy_price'], $data['sell_price'], $data['stock_qty'], $data['alert_threshold'], $data['image']],
                "isiidddis"
            );
        }
    }
