...
<body>
    <div class="dashboard-layout">
        ...
        <div class="main-content">
            ...
            <form class="form-group"
                style="display: flex; gap: 1rem; align-items: flex-end; background: var(--bg-card); padding: 1rem; border-radius: 0.5rem; margin-bottom: 2rem;">
                <div>
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <h3>Net Profit</h3>
                    <div class="stat-value" style="color: #34d399;">৳<?php echo number_format($total_profit, 2); ?>
                    </div>
                </div>
                <div class="stat-card">
                    <h3>Orders Count</h3>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value" style="font-size: 1.5rem; color: var(--primary);">
                        <?php echo htmlspecialchars($best_seller['name']); ?>
                    </div>
            ...
            <!-- AI Section -->
            <div
                style="margin-top: 2rem; background: linear-gradient(135deg, rgba(79, 70, 229, 0.1) 0%, rgba(6, 182, 212, 0.1) 100%); padding: 1.5rem; border-radius: 1rem; border: 1px solid rgba(79, 70, 229, 0.2);">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <h2
                            style="background: linear-gradient(135deg, #4f46e5 0%, #06b6d4 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">
                            AI Business Analyst</h2>
                        <p style="color: var(--text-gray);">Get insights and tips to increase your sales based on this
                            report.</p>
                    </div>
                    <button id="askAiBtn" onclick="fetchAiInsights()" class="btn-primary" style="width: auto;">✨
                        Generate Insights with Gemini</button>
                </div>
            ...
            <div class="table-container">
                <?php if (isset($success)): ?>
                    <div
                        style="background: rgba(52,211,153,0.1); color: #34d399; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1rem;">
                        <?php echo $success; ?>
                    </div>
                <?php endif; ?>
                <table>
                    ...
                    <tbody>
                        <?php foreach ($orders_data as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <button onclick="showDetails(<?php echo $row['id']; ?>)" class="btn-primary"
                                        style="padding: 0.25rem 0.5rem; font-size: 0.8rem; width: auto;">Details</button>
                                    <form method="POST" style="display: inline;"
                                        onsubmit="return confirm('Return this order? Stock will be restored.');">
                                        <input type="hidden" name="order_id" value="<?php echo $row['id']; ?>">
                                        <button type="submit" name="return_order"
                                            style="color: #ef4444; background: none; border: none; cursor: pointer; margin-left: 0.5rem;">Return</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <div id="detailsModal"
        ...
        <div style="background: var(--bg-card); padding: 2rem; border-radius: 1rem; width: 500px; max-width: 90%;">
            ...
            <button onclick="document.getElementById('detailsModal').style.display='none'" class="btn-primary"
